...
from app import db, login, flask_app

# Auxiliary many-to-many relationship table that keeps track of followers
followers = sa.Table(
    "followers",
    db.metadata,
...
class Post(db.Model):
    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    body: so.Mapped[str] = so.mapped_column(sa.String(512), index=True, unique=True)
